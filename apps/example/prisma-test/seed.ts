...
try {
  ...
  await prisma.orderItem.deleteMany({
    where: { order: { tenantId: { in: tenantIds } } },
  });
      ...
      const role =
        tenant.name === "Northwind" && index === 2
          ...
          : index === 0 || (tenantIndex === 2 && index === 3)
            ...
            : Role.USER;
      return {
        id: `example-user-${tenantIndex + 1}-${index + 1}`,
        email:
  ...
  const customers = tenants.flatMap((tenant, tenantIndex) =>
    customerNames.map((fullName, index) => ({
      id: `example-customer-${tenantIndex + 1}-${index + 1}`,
  ...
  const categories = tenants.flatMap((tenant, tenantIndex) =>
    categoryNames.map((name, index) => ({
      id: `example-category-${tenantIndex + 1}-${index + 1}`,
      name,
  ...
  const products = tenants.flatMap((tenant, tenantIndex) =>
    Array.from({ length: 40 }, (_, index) => {
      ...
      return {
        id: `example-product-${tenantIndex + 1}-${index + 1}`,
  ...
  const posts = tenants.flatMap((tenant, tenantIndex) =>
    Array.from({ length: 40 }, (_, index) => ({
      id: `example-post-${tenantIndex + 1}-${index + 1}`,
      ...
      published: index % 4 !== 0,
      authorId: users[tenantIndex * names.length + (index % names.length)].id,
      tenantId: tenant.id,
  ...
  for (const [tenantIndex, tenant] of tenants.entries()) {
    ...
    for (let index = 0; index < 30; index += 1) {
      ...
      await prisma.order.create({
        data: {
          id: `example-order-${tenantIndex + 1}-${index + 1}`,
  ...
  console.log(
    `Seeded ${tenants.length} tenants, ${users.length} users, ${customers.length} customers, ${categories.length} categories, ${products.length} products, ${posts.length} posts, ${orderCount} orders, and ${itemCount} order items.`,
  );
} finally {
