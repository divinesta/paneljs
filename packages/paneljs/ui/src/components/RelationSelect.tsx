...
export const RelationSelect = ({
  ...
  useEffect(() => {
    ...
    const timeout = window.setTimeout(() => {
      setStatus("loading");
      fetch(
        `${apiBase}/${model.pluralName}?${new URLSearchParams({ search, page: "1" })}`,
        {
          credentials: "include",
        ...
        .catch((reason: unknown) => {
          ...
          setMessage(
            reason instanceof Error
              ? reason.message
              : "Could not search related records.",
          );
          setStatus("error");
        });
    }, 250);
  ...
  const selectRecord = (record: RecordData) => {
    ...
    const displayValue = String(
      record[model.displayField] ?? record[model.idField],
    );
    setQuery("");
    ...
    <div className={`relation-select ${error ? "has-error" : ""}`}>
      ...
      {!value && (
        <span className="relation-help" id={`${inputId}-help`}>
          Type at least two characters, then choose a result.
        </span>
      )}
      {status === "loading" && (
        <span className="relation-help">Searching…</span>
      )}
      {status === "error" && <span className="form-error">{message}</span>}
      {status === "ready" && records.length === 0 && (
        <span className="relation-help">No matching records found.</span>
      )}
      {!readOnly && records.length > 0 && (
        <ul
          className="relation-results"
          id={listboxId}
          role="listbox"
