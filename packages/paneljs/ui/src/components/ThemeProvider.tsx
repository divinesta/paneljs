...
const readAppearance = (): Appearance => {
  try {
    ...
    return {
      ...
      palette:
        stored.palette && stored.palette in paletteLabels
          ? stored.palette
...
const useTheme = () => {
  ...
  if (!theme)
    throw new Error("ThemeSettings must be rendered within ThemeProvider.");
  return theme;
};
...
export const ThemeSettings = ({
  ...
  return (
    <div className="appearance-control" ref={controlRef}>
      <button
        className="appearance-trigger identity-chip"
        type="button"
        aria-expanded={open}
            ...
            <div className="palette-options">
              {(Object.keys(paletteLabels) as PaletteName[]).map((option) => (
                <button
                  key={option}
                  type="button"
                  className={`palette-swatch ${option} ${palette === option ? "is-selected" : ""}`}
                  aria-label={`${paletteLabels[option]} palette`}
