...
export const DeleteConfirmationPage = ({ schema }: { schema: Schema }) => {
  ...
  useEffect(() => {
    ...
    setStatus("loading");
    fetch(
      `${apiBase}/${model.meta.pluralName}/actions/delete-preview?${new URLSearchParams({ ids: ids.join(",") })}`,
      {
        credentials: "include",
      ...
      .catch((reason: unknown) => {
        ...
        setError(
          reason instanceof Error
            ? reason.message
            : "Could not load delete confirmation.",
        );
        setStatus("error");
      });
  ...
  if (status === "loading")
    return (
      <FullPageState
        ...
        title="Checking related records"
        detail="Preparing the records that will be affected..."
        busy
      />
    );

  const listFields = model.config.listDisplay
    .map((name) => model.meta.fields.find((field) => field.name === name))
  ...
  const recordLabel =
    ids.length === 1 ? model.meta.name : `${model.meta.name} records`;
    ...
    <section className="page-section delete-confirm-page">
      <div className="delete-confirm-layout">
          ...
          <div className="delete-confirm-heading">
            ...
            <div>
              <div className="eyebrow">
                {blocked ? "Deletion blocked" : "Confirm deletion"}
              </div>
              <h1>
                {blocked
                  ? `Can't delete ${ids.length === 1 ? `this ${model.meta.name}` : `these ${model.meta.name} records`}`
                  : `Delete ${ids.length} ${recordLabel}?`}
              </h1>
              <p>
                {blocked
        ...
        <div className="delete-confirm-actions">
          <NavLink className="secondary-button" to={cancelPath}>
            Cancel
          </NavLink>
          <button
            ...
            type="button"
            disabled={
              blocked || status !== "ready" || bulkActions.status === "running"
            }
            onClick={() => void confirmDelete()}
          >
            <Trash2 size={14} strokeWidth={2} aria-hidden />
            {bulkActions.status === "running"
              ? "Deleting..."
              : "Confirm delete"}
          </button>
        </div>
      ...
      {status === "ready" && preview && (
        <div className="delete-tree">
          {preview.records.map((record) => (
            <DeleteTreeItem
              key={String(record[model.meta.idField])}
              model={model}
              record={record}
...
const DeleteTreeItem = ({
  ...
  const label = displayField
    ? formatRecordValue(record[displayField.name], displayField)
    : id;
  ...
  return (
    <article className="delete-tree-item">
      <div className="delete-parent-row">
        ...
        <span>
          {fieldLabel(model.meta.idField)}: {id}
        </span>
      </div>
            ...
            {children.map((child) => {
              ...
              return (
                <li key={childId}>
                  ...
                  <div>
                    <strong>
                      {relation.modelName}: {String(childValue)}
                    </strong>
                    <span
                      className={`delete-relation-tag delete-relation-tag-${relation.onDelete.toLowerCase()}`}
                    >
                      {relationActionLabel(relation.onDelete)}
                    </span>
                    <span>
                      {fieldLabel(relation.idField)}: {childId}
                    </span>
                  </div>
                </li>
              );
