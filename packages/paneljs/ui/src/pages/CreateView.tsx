...
export const CreateView = ({
  ...
  const model = schema.models.find(
    (candidate) => candidate.meta.pluralName === modelPath,
  );
  const fields = model
    ? mode === "view"
      ? model.meta.fields.filter(
          (field) => field.type !== "relation" && !field.isList,
        )
      : writableFields(model.meta.fields)
    : [];
  const relationModelsByForeignKey = new Map(
    model?.meta.fields.flatMap((field) => {
      ...
      if (
        field.type !== "relation" ||
        !relation ||
        (relation.kind !== "belongsTo" && relation.kind !== "hasOne") ||
        relation.foreignKeyFields.length !== 1
      )
        return [];
      const relatedModel = schema.models.find(
        (candidate) => candidate.meta.name === relation.model,
      );
      if (!relatedModel?.config.permissions.list) return [];
      return [
        [
          relation.foreignKeyFields[0],
          {
            label: fieldLabel(field.name),
            pluralName: relatedModel.meta.pluralName,
            idField: relatedModel.meta.idField,
    ...
    fetchRecord(model.meta.pluralName, id)
      .then((record) => {
        ...
        relationModelsByForeignKey.forEach((relationModel, foreignKeyField) => {
          const relationField = model.meta.fields.find(
            (field) =>
              field.type === "relation" &&
              field.relation?.foreignKeyFields.includes(foreignKeyField),
          );
          const relationRecord = relationField
            ? record[relationField.name]
            : undefined;
          if (
            typeof relationRecord !== "object" ||
            relationRecord === null ||
            Array.isArray(relationRecord)
          )
            return;
          const relationData = relationRecord as Record<string, unknown>;
          const label =
            relationData[relationModel.displayField] ?? record[foreignKeyField];
  ...
  if (mode === "create" && !model.config.permissions.create)
    return <NotFound />;
  ...
  const submit = async (event: FormEvent) => {
    ...
    form.setError("");
    form.setFieldErrors({});
    const missingRequiredRelation = fields.find(
      (field) =>
        relationModelsByForeignKey.has(field.name) &&
        field.isRequired &&
        form.values[field.name] === "",
    );
    if (missingRequiredRelation) {
      form.setFieldErrors({
        [missingRequiredRelation.name]: `${fieldLabel(missingRequiredRelation.name)} is required.`,
      });
      form.setStatus("ready");
      return;
    }
    ...
    fields.forEach((field) => {
      ...
      if (value === "" && !field.isRequired) {
        if (relationModelsByForeignKey.has(field.name))
          payload[field.name] = null;
        return;
      }
      payload[field.name] =
        field.type === "number" &&
        field.nativeType !== "Decimal" &&
        field.nativeType !== "BigInt"
          ? Number(value)
    ...
    const url =
      mode === "create"
        ? `${apiBase}/${model.meta.pluralName}`
        : `${apiBase}/${model.meta.pluralName}/${encodeURIComponent(id ?? "")}`;
    ...
    <section className="page-section">
      <div className="page-heading">
        <div>
          ...
          <h1>
            {mode === "create"
              ? `Create ${model.meta.name}`
              : mode === "view"
                ? model.meta.name
                : `Edit ${model.meta.name}`}
          </h1>
          <p>
            {mode === "view"
      ...
      <form className="record-form" onSubmit={submit}>
        ...
        <div className="form-actions">
          <button
            ...
            type="button"
            onClick={() => navigate(`/${model.meta.pluralName}`)}
          >
