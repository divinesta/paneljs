...
function assertNoUndefinedScopeValues(
  value: unknown,
  path: string[] = [],
): void {
  if (value === undefined) {
    ...
    throw new RequestValidationError(
      `Scope field "${label}" resolved to undefined. Return a concrete value or a match-nothing fallback instead.`,
    );
  }

  if (Array.isArray(value)) {
    value.forEach((item, index) =>
      assertNoUndefinedScopeValues(item, [...path, String(index)]),
    );
    return;
  }
 ...
 */
export function buildScopedRecordWhere(
  scope: ScopeFilter,
  idField: string,
  id: unknown,
): ScopeFilter {
...
export function applyCreateScope(
  ...
  for (const [fieldName, value] of Object.entries(scope)) {
    if (value !== null && typeof value === "object") {
      throw new RequestValidationError(
        `Cannot apply complex scope field "${fieldName}" when creating a record. Use a simple equality scope or a beforeCreate hook.`,
      );
    }

    if (
      scopedData[fieldName] !== undefined &&
      scopedData[fieldName] !== value
    ) {
      throw new RequestValidationError(
        `Create payload conflicts with the configured scope for field "${fieldName}".`,
      );
    }
...
export function assertScopeFieldsUnchanged(
  data: Record<string, unknown>,
  scope: ScopeFilter,
): void {
  for (const fieldName of collectScopeFieldNames(scope)) {
    if (data[fieldName] !== undefined) {
      throw new RequestValidationError(
        `Field "${fieldName}" is controlled by the configured scope and cannot be updated through the admin.`,
      );
    }
  }
}
