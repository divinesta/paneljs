...
function relationSelects(meta: AdminModelMeta, model: FullRegisteredModel, allowedNames?: Set<string>): FieldSelect["relations"] {
   ...
   for (const fieldName of model.resolved.listDisplay) {
      ...
      relations.push({ field: field.name, displayField: field.relation.displayField });
   }
   return relations;
}

/** Build the safe select used by detail and edit endpoints. */
export function buildRecordSelect(meta: AdminModelMeta, model: FullRegisteredModel): FieldSelect {
   const fields = meta.fields
      .filter((field) => field.type !== "relation" && isFieldVisible(field, model.raw))
...
export function buildListRecordSelect(meta: AdminModelMeta, model: FullRegisteredModel): FieldSelect {
   const allowed = new Set([meta.idField, ...model.resolved.listDisplay]);
   const fields = meta.fields
      .filter((field) => field.type !== "relation" && allowed.has(field.name) && isFieldVisible(field, model.raw))
      .map((field) => field.name);
   if (!fields.includes(meta.idField)) fields.unshift(meta.idField);
