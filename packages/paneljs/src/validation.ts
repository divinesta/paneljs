...
export function isSensitiveFieldName(name: string): boolean {
  return /password|token|secret|api[_-]?key|credential|private[_-]?key/i.test(
    name,
  );
}
...
export function getWritableFields(
  ...
  return meta.fields.filter((field) =>
    isFieldWritableByConfiguration(field, config),
  );
}

function isFieldWritableByConfiguration(
  field: AdminFieldMeta,
  config: ModelConfig,
): boolean {
  const override = config.fields?.[field.name];
  return (
    isFieldVisible(field, config) &&
    field.type !== "relation" &&
    !field.isList &&
    !field.isReadOnly &&
    !override?.readOnly
...
function assertFieldValue(field: AdminFieldMeta, value: unknown): void {
  if (value === null) {
    if (field.isRequired)
      throw new RequestValidationError(`Field "${field.name}" cannot be null.`);
    return;
  }
  ...
  switch (field.type) {
    ...
    case "number":
      if (field.nativeType === "Decimal" || field.nativeType === "BigInt") {
        if (typeof value !== "string" || !/^-?\d+(?:\.\d+)?$/.test(value))
          throw new RequestValidationError(
            `Field "${field.name}" must be a decimal string.`,
          );
        return;
      }
    ...
    case "datetime":
      if (typeof value !== "string" || Number.isNaN(Date.parse(value)))
        throw new RequestValidationError(
          `Field "${field.name}" must be an ISO date-time string.`,
        );
      return;
    case "enum":
      if (typeof value !== "string" || !field.enumValues?.includes(value))
        throw new RequestValidationError(
          `Field "${field.name}" must be a valid ${field.nativeType} value.`,
        );
      return;
    case "json":
      if (!isJsonValue(value))
        throw new RequestValidationError(
          `Field "${field.name}" must be valid JSON data.`,
        );
      return;
    case "relation":
      throw new RequestValidationError(
        `Relation field "${field.name}" is not supported for writes yet.`,
      );
  }
}
...
export function assertRequiredCreateFields(
  ...
  for (const field of meta.fields) {
    ...
    if (data[field.name] === undefined)
      throw new RequestValidationError(`Field "${field.name}" is required.`);
  }
}
...
export function validateHookPayload(
  ...
  if (!isPlainObject(body))
    throw new RequestValidationError("Hook output must be a JSON object.");
  const fieldsByName = new Map(
    meta.fields
      .filter(
        (field) =>
          field.type !== "relation" && !field.isList && !field.isReadOnly,
      )
  ...
  for (const [fieldName, value] of Object.entries(body)) {
    ...
    if (!field)
      throw new RequestValidationError(
        `Hook output field "${fieldName}" cannot be written.`,
      );
...
export function validateWritePayload(
  ...
  const writableByName = new Map(
    getWritableFields(meta, config)
      .filter((field) => isFieldWritable(field, config, adminUser))
      .map((field) => [field.name, field]),
  );
  const data: Record<string, unknown> = {};
  ...
  for (const [fieldName, value] of Object.entries(body)) {
    ...
    if (!field)
      throw new RequestValidationError(
        `Field "${fieldName}" cannot be written through the admin.`,
      );
