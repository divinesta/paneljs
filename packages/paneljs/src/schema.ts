...
export function buildSchemaResponse(
  ...
  return {
    ...
    models: registry
      .getAll()
      .filter(({ resolved }) =>
        hasModelPermission(adminUser, resolved.permissions, "list"),
      )
      .map(({ meta, resolved, raw }) => {
        const visibleFields = meta.fields
          ...
          .map((field) => ({
            ...field,
            isReadOnly:
              field.isReadOnly || !isFieldWritable(field, raw, adminUser),
          }));
        const visibleFieldNames = new Set(
          visibleFields.map((field) => field.name),
        );
        const canDelete = hasModelPermission(
          adminUser,
        ...
        const actions = canDelete
          ? [DELETE_SELECTED_ACTION, ...customActions]
          : customActions;
        ...
        return {
          meta: {
            ...
            searchableFields: meta.searchableFields.filter((fieldName) =>
              visibleFieldNames.has(fieldName),
            ),
            filterableFields: meta.filterableFields.filter((fieldName) =>
              visibleFieldNames.has(fieldName),
            ),
          },
          config: {
            listDisplay: resolved.listDisplay.filter((fieldName) =>
              visibleFieldNames.has(fieldName),
            ),
            listFilter: resolved.listFilter.filter((fieldName) =>
              visibleFieldNames.has(fieldName),
            ),
            searchFields: resolved.searchFields.filter((fieldName) =>
              visibleFieldNames.has(fieldName),
            ),
            ...
            permissions: {
              ...
              create: hasModelPermission(
                adminUser,
                resolved.permissions,
                "create",
              ),
              update: hasModelPermission(
                adminUser,
                resolved.permissions,
                "update",
              ),
