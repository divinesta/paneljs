...
export function hasModelPermission(
  ...
  if (allowedRoles === undefined)
    return operation === "list" || operation === "view";
...
export function hasRegisteredActionPermission(
  adminUser: AdminUser,
  permissions: ModelPermissions,
  action: AdminAction,
): boolean {
  ...
  if (modelAllowedRoles === undefined && actionAllowedRoles === undefined)
    return false;
  return (
    (modelAllowedRoles === undefined ||
      modelAllowedRoles.includes(adminUser.role)) &&
    (actionAllowedRoles === undefined ||
      actionAllowedRoles.includes(adminUser.role))
  );
}
