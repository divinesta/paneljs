...
import type { AdminHttpRequest, AdminUser, BuiltInAuthConfig } from "./types.js";
...
export function sessionCookie(token: string, config: BuiltInAuthConfig, basePath: string, expires: Date): SessionCookie {
   return {
...
export function clearedSessionCookie(config: BuiltInAuthConfig, basePath: string): SessionCookie {
   return {
      name: SESSION_COOKIE_NAME,
      value: "",
      path: basePath,
...
export function builtInUserToAdminUser(user: BuiltInUserRecord): AdminUser | null {
...
export function resolveAuthStore(adapter: DataAdapter, auth: BuiltInAuthConfig): AdminAuthStore {
...
export async function authenticateSession(store: AdminAuthStore, token: string | null): Promise<AdminUser | null> {
...
export async function authenticateBuiltInRequest(req: AdminHttpRequest, store: AdminAuthStore): Promise<AdminUser | null> {
...
export async function loginWithPassword(
   ...
   const adminUser = user && passwordMatches ? builtInUserToAdminUser(user) : null;
   ...
   await store.createSession({ tokenHash: hashSessionToken(token), userId: user.id, expiresAt });
   return {
      ok: true,
      status: 200,
      body: { ok: true },
