...
import type { DataAdapter, FullRegisteredModel, AuditConfig } from "paneljs";
import {
   writeAuditEvent,
   RecordNotFoundError,
   buildListWhere,
...
export function createCrudRouter(models: Map<string, FullRegisteredModel>, adapter: DataAdapter, databaseProvider?: string, audit?: AuditConfig): Router {
   const router = Router();
   ...
   router.get("/:model", route(async (req, res) => {
      ...
      const { page, sort, dir, filters, search } = parseListQuery(req.query as Record<string, string | string[] | undefined>, model.meta, model, databaseProvider);
   ...
   router.post("/:model", route(async (req, res) => {
      ...
      await assertSelectedRelationsAreVisible(data, model, models, adapter, adminUser);
      ...
      const record = await adapter.resource(model.meta).create({ data, select: buildRecordSelect(model.meta, model) });
   ...
   router.put("/:model/:id", route(async (req, res) => {
      ...
      if (!record) throw new Error(`[paneljs] Updated record "${model.meta.name}/${id}" could not be reloaded.`);
...
async function runPostCommit(name: string, task: () => Promise<void>): Promise<void> {
   try { await task(); }
   catch { console.error(`[paneljs] ${name} failed after the database write committed.`); }
}
