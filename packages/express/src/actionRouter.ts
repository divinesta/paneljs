...
export function createActionRouter(models: Map<string, FullRegisteredModel>, adapter: DataAdapter, audit?: AuditConfig): Router {
   const router = Router();
   ...
   router.post("/:model/actions/:action", route(async (req, res) => {
         ...
         const result = await action.handler({ ids, adminUser, client: adapter.client, where });
...
async function writeAuditSafely(audit: AuditConfig | undefined, actor: import("paneljs").AdminUser, event: Parameters<typeof writeAuditEvent>[2]): Promise<void> {
