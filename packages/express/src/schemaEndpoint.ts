...
import { AuthenticationError, buildSchemaResponse, isBuiltInAuth, type Admin } from "paneljs";
import { sendApiError } from "./httpErrors.js";

export function createSchemaEndpoint(admin: Admin): RequestHandler {
   const basePath = admin.config.basePath && admin.config.basePath.length > 1 ? admin.config.basePath.replace(/\/+$/, "") : admin.config.basePath ?? "/admin";
