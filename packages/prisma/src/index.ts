import type { AdminModelMeta, DataAdapter, ModelResource } from "paneljs";
...
function getDelegate(prisma: PrismaClientLike, meta: AdminModelMeta): PrismaModelDelegate {
   const delegate = (prisma as Record<string, PrismaModelDelegate | undefined>)[meta.clientKey];
   if (!delegate) throw new Error(`[paneljs] Prisma client has no delegate for model "${meta.name}".`);
   return delegate;
}
