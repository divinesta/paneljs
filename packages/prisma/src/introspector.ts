...
const { getDMMF } = createRequire(import.meta.url)(
  "@prisma/internals",
) as typeof import("@prisma/internals");
...
function detectDisplayField(fields: DMMF.Field[]): string {
  ...
  for (const preferred of preferredNames) {
    const field = fields.find(
      (f) =>
        f.name === preferred && f.type === "String" && f.isUnique && !f.isId,
    );
...
function introspectField(
  ...
  const fieldMeta: AdminFieldMeta = {
    ...
    enumValues:
      field.kind === "enum" ? enumValuesMap.get(field.type) : undefined,
    relation: null,
  };
...
function introspectModel(
  ...
  if (model.primaryKey !== null) {
    console.warn(
      `[paneljs] Skipping model "${model.name}": composite primary keys (@@id) are not supported in v1.`,
    );
    return null;
  }
  ...
  if (!pkField) {
    console.warn(
      `[paneljs] Skipping model "${model.name}": no @id field found.`,
    );
    return null;
  }
  ...
  const fields = model.fields.map((f) =>
    introspectField(f, enumValuesMap, allModelNames),
  );

  // ── Timestamps ─────────────────────────────────────────────
  const createdAtField = model.fields.find(
    (f) => f.name === "createdAt" && f.type === "DateTime",
  );
  const updatedAtField = model.fields.find(
    (f) => (f.name === "updatedAt" || f.isUpdatedAt) && f.type === "DateTime",
  );
    ...
    for (const field of model.fields) {
      if (field.type === "relation" && field.relation) {
        ...
        if (relatedModel) {
          ...
          (field.relation as { displayField: string }).displayField =
